...
input_file = "../input/tweet_keyword_mapping_pull_factors_environment_health.json"
output_file = "../output/augmented_pull_factors_environment_health.json"

label = "pull_factors_environment_health"
...
def extract_rationale(tweet):
    ...
    if response and response.status_code == 200:
        try:
            llm_output = response.json()["choices"][0]["message"]["content"]
            rationale = json.loads(llm_output)  # Parse JSON array
